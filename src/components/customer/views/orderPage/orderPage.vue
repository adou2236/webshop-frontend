    ...
    <div id="all">
        ...
        <div class="mainArea">
          ...
          <div class="localList">
            <div class="resieverBox" :class="flag===item._id?'choosed':''" v-for="item in Resiver" :key="item._id">
              <div @click="changeLocation(item._id)" style="text-align:left;box-sizing: border-box;padding:0 20px;width: 100%;height: 100%">
                <span class="checkpoint" v-if="flag===item._id"><i style="color: #5683ea;font-size: 20px" class="el-icon-success"></i></span>
                <p>收件地址：{{item.areaAddress}}</p>
                <p>详细地址：{{item.detailAddress}}</p>
                <p>收件人：{{item.name}}</p>
                <p>联系电话：{{item.phone}}</p>
              </div>
            </div>
            <div style="line-height: 150px;font-size: 20px" class="resieverBox" @click="dialogFormVisible=true"><i class="el-icon-circle-plus">添加新地址</i></div>
          </div>
        </div>
        <div class="mainArea">
      ...
      <el-dialog title="新增收货地址"
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="addNewResiver">确 定</el-button>
        </div>
      </el-dialog>
    </div>
</template>
...
<script>
  ...
  import {getGood, ceateOrder, getResiver, updateAddress} from '../../../../apis/customer'
